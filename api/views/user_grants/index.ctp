<div class="userGrants index">
	<h2><?php __('User Grants');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('user_type_id');?></th>
			<th><?php echo $this->Paginator->sort('master_module_id');?></th>
			<th><?php echo $this->Paginator->sort('created');?></th>
			<th><?php echo $this->Paginator->sort('modified');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($userGrants as $userGrant):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $userGrant['UserGrant']['id']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($userGrant['UserType']['name'], array('controller' => 'user_types', 'action' => 'view', $userGrant['UserType']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($userGrant['MasterModule']['name'], array('controller' => 'master_modules', 'action' => 'view', $userGrant['MasterModule']['id'])); ?>
		</td>
		<td><?php echo $userGrant['UserGrant']['created']; ?>&nbsp;</td>
		<td><?php echo $userGrant['UserGrant']['modified']; ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $userGrant['UserGrant']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $userGrant['UserGrant']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $userGrant['UserGrant']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $userGrant['UserGrant']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< ' . __('previous', true), array(), null, array('class'=>'disabled'));?>
	 | 	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true) . ' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New User Grant', true), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List User Types', true), array('controller' => 'user_types', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New User Type', true), array('controller' => 'user_types', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Master Modules', true), array('controller' => 'master_modules', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Master Module', true), array('controller' => 'master_modules', 'action' => 'add')); ?> </li>
	</ul>
</div>