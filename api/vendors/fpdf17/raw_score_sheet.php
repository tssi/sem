...
require(__DIR__.'/fpdf.php');
	class RawScoreSheet extends FPDF{
		...
		public function create($school_name,$teacher,$sy,$section,$deptalias,$subj_nomen,$period, $curr_page, $total_page,$flag=0){
			$this->AddPage();
			$this->SetMargins(0,0);
			//$this->SetLeftMargin(-0.5);
			$this->LogoHead($school_name,$teacher,$sy,$section,$deptalias,$subj_nomen,$period,$curr_page, $total_page,$flag);
			} 	
		public function LogoHead($school_name,$teacher,$sy,$section,$deptalias,$subj_nomen,$period,$curr_page, $total_page,$flag){
			if(is_numeric($period)){
				switch($period){
					case 1: $period_str = "Midterm  / First Sem"; break;
					case 2: $period_str = "Finals  / First Sem"; break;
					case 3: $period_str = "Midterm  / Second Sem"; break;
					case 4: $period_str = "Finals  / Second Sem"; break;
					case 5: $period_str = "Final Rating"; break;
					case 25: $period_str = "First Sem"; break;
					case 45: $period_str = "Second Sem"; break;
				}
				$period_str ="";
			}else{
				$period_str =  $period;
			...
			if($flag==0){
				...
				$this->CenterText($center_text,0.50,$school_name.' '.$deptalias);
				...
				$MANILA_TIME = date("Y-m-d h:i:s A", time());
		...
		public function PlotColHeaders($measurables, $components_rec, $v_lines, $h_lines, $cellwidth, $cellheight, $cocurratt=false, $FONT_SIZE = 10,$disp_flg=null ){
			...
			foreach($components_rec as $c){
				...
				for($i=0;$i<$c['count']-1;$i++){
					...
					$fsize = $FONT_SIZE *0.7;
					$cWidth = round($cellwidth*0.80,2);
					do{				
						$this->SetFont('Arial','',$fsize);
						$const2 =round($this->GetStringWidth($disp),2);
						$fsize-=0.1;
					}while($const2>$cWidth);

					$this->MultiCell($cellwidth,$cellheight, $disp,0,'C'); // Column Label
					//$this->SetXY($v_lines[$col_ctr+$i],0.8);
					//$this->MultiCell($cellwidth,$cellheight, $cWidth.' '.$const2,$i%2,'C'); // Column Label
		...
		public function PlotStudents($students, $start_at, $h_lines, $width, $height,$offset=0,$rawscore, $summary, $components,$v_lines,$col_hdrs,$cellwidth,$cellheight, $cocurratt=false, $start_index=0, $g_flg='', $FONT_SIZE, $ROWS,$is_gs = false,$disp_flg=null){
			...
			do{
				...
				if($curr_g_flg != $prev_g_flg ){
					...
					$this->SetXY(0.35,$h_lines[$line_ctr]+$offset);
					$this->MultiCell($width,$height,$gender_label);	
					$line_ctr+=1;
				}
				$prev_g_flg = $curr_g_flg; 
				//LRN
				$hzl = $h_lines[$line_ctr]+$offset;
				$fsize =7;
				$this->SetFont('Arial','',$fsize);
				//$this->Line($start_at,$hzl,$start_at,$hzl+$height);
				$this->SetXY(0.35,$h_lines[$line_ctr]+$offset);
				//pr($students[$index]);
				//$this->MultiCell($width,$height,($students[$index]['lrn']));
				//NAME
				...
				$this->MultiCell($width,$height,utf8_decode($students[$index]['fullname']));
